...
export const decisionContextStatusEnum = pgEnum('decision_context_status', ['drafting', 'reviewing', 'locked', 'logged']);
export const decisionMethodEnum = pgEnum('decision_method', ['consensus', 'vote', 'authority', 'defer', 'reject']);
export const fieldCategoryEnum = pgEnum('field_category', ['context', 'evaluation', 'outcome', 'metadata']);
export const fieldTypeEnum = pgEnum('field_type', ['text', 'textarea', 'select', 'multiselect', 'number', 'date', 'url']);
export const templateCategoryEnum = pgEnum('template_category', ['standard', 'technology', 'strategy', 'budget', 'policy', 'proposal']);
export const expertTypeEnum = pgEnum('expert_type', ['technical', 'legal', 'stakeholder', 'custom']);
...
export const transcriptChunks = pgTable('transcript_chunks', {
  id: uuid('id').primaryKey().defaultRandom(),
  meetingId: uuid('meeting_id').notNull().references(() => meetings.id),
  rawTranscriptId: uuid('raw_transcript_id').notNull().references(() => rawTranscripts.id),
...
export const chunkRelevance = pgTable('chunk_relevance', {
  id: uuid('id').primaryKey().defaultRandom(),
  chunkId: uuid('chunk_id').notNull().references(() => transcriptChunks.id),
...
export const templateFieldAssignments = pgTable('template_field_assignments', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const decisionContexts = pgTable('decision_contexts', {
  id: uuid('id').primaryKey().defaultRandom(),
  meetingId: uuid('meeting_id').notNull().references(() => meetings.id),
  flaggedDecisionId: uuid('flagged_decision_id').notNull().references(() => flaggedDecisions.id),
  title: text('title').notNull(),
  templateId: uuid('template_id').notNull().references(() => decisionTemplates.id),
