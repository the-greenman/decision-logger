...
export async function startWebServer(options?: StartWebServerOptions): Promise<RunningWebServer> {
  ...
  const maxChunkBytes = options?.maxChunkBytes ?? parsePositiveInt(process.env.TRANSCRIPTION_MAX_CHUNK_BYTES, 25_000_000);
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (method === 'POST' && chunkMatch) {
        ...
        const normalizedEvents = normalizeSequenceNumbers(transcription.events, session.nextSequenceNumber);
