...
function printUsage(): void {
  console.log(
    "Usage: transcription-service transcribe <audio-file> --meeting-id <uuid> [--api-url <url>] [--language <code>] [--mode upload|stream] [--chunk-strategy fixed|semantic|speaker|streaming]",
  );
  console.log(
    "       transcription-service transcribe-local <audio-file> [--language <code>] [--output <path>] [--output-text <path>] [--output-srt <path>]",
  );
  console.log(
    "       transcription-service smoke-upload <audio-file> [--meeting-id <uuid>] [--api-url <url>] [--language <code>] [--chunk-strategy fixed|semantic|speaker|streaming]",
  );
  console.log(
...
async function main(): Promise<void> {
  ...
  if (parsed.command === "transcribe-local") {
    ...
    await runLocalTranscription({
      ...
      ...(parsed.outputTextPath === undefined
        ? {}
        : { outputTextPath: resolve(parsed.outputTextPath) }),
      ...(parsed.outputSrtPath === undefined
        ? {}
        : { outputSrtPath: resolve(parsed.outputSrtPath) }),
    });
    return;
  }
