...
const RELATION_ORDER: RelationType[] = ['related', 'blocks', 'blocked_by', 'supersedes', 'superseded_by'];
...
export function RelationsAccordion({
  ...
  if (relations.length === 0) {
    return <p className={cn('text-text-muted italic', density === 'display' ? 'text-display-meta' : 'text-fac-meta', className)}>{emptyLabel}</p>;
  }
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      <p className={cn(density === 'display' ? 'text-display-label' : 'text-fac-label', 'text-text-secondary uppercase tracking-widest')}>
        {title}
      </p>
          ...
          >
            <div className="flex flex-col gap-2">
              {group.items.map((relation) => (
                <article key={relation.id} className="rounded border border-border bg-surface px-3 py-2.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={cn('inline-flex items-center px-1.5 py-0.5 rounded-badge border text-[11px] font-medium', config.badgeClass)}>
                      {config.label}
                    </span>
                  ...
                  <p className={cn('text-text-primary mt-1.5 leading-relaxed', density === 'display' ? 'text-display-meta' : 'text-fac-field')}>
