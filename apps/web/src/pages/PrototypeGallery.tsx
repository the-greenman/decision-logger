      ...
      <main className="max-w-5xl mx-auto px-6 py-8 flex flex-col gap-12">
        ...
        <Section title="Flow 1 — Offline transcript, known decision" icon={null}>
          ...
          <ol className="flex flex-col gap-2 text-fac-meta">
            <FlowStep
              ...
              to="/"
              note="New meeting form → add participants"
            />
            <FlowStep
              n={2}
              label="Upload transcript"
              to="/meetings/mtg-1/facilitator"
              note="Header → Upload transcript button"
            />
            <FlowStep
              n={3}
              label="Review AI candidates"
              to="/meetings/mtg-1/facilitator"
              note="Suggested tab — dismiss noise"
            />
            <FlowStep
              n={4}
              label="Create manual context (G2)"
              to="/meetings/mtg-1/facilitator"
              note="Header → New decision button → template picker"
            />
            <FlowStep
              n={5}
              label="Select transcript segments (G3)"
              to="/meetings/mtg-1/facilitator/transcript"
              note="Row jump + drag-select"
            />
            <FlowStep
              n={6}
              label="Generate draft"
              to="/meetings/mtg-1/facilitator"
              note="Header → Regenerate"
            />
            <FlowStep
              n={7}
              label="Shared display (projected)"
              to="/meetings/mtg-1"
              note="Projection view — read-only"
            />
            <FlowStep
              n={8}
              label="Zoom into field + add evidence (G4)"
              to="/meetings/mtg-1/facilitator"
              note="Click zoom icon on Options field"
            />
            <FlowStep
              n={9}
              label="Regenerate with focus (G5 resolved)"
              to="/meetings/mtg-1/facilitator"
              note="Regenerate dialog → focus note"
            />
            <FlowStep
              n={10}
              label="Finalise — unanimous vote"
              to="/meetings/mtg-1/facilitator"
              note="Finalise button → method + actors"
            />
        ...
        <Section title="FieldCard — display density" icon={<Monitor size={14} />}>
          <div className="flex flex-col gap-4">
            ...
            <FieldCard
              field={{ ...ACTIVE_CONTEXT.fields[2]!, status: "generating" }}
              density="display"
            />
        ...
        <Section
          title="FacilitatorFieldCard — with zoom, lock, supplementary indicator"
          icon={<Laptop size={14} />}
        >
          <div className="flex flex-col gap-3 max-w-xl">
            <FacilitatorFieldCard field={ACTIVE_CONTEXT.fields[0]!} supplementaryCount={0} />
            <FacilitatorFieldCard
